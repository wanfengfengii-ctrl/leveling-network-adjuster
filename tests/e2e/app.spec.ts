...
test.describe('隧道复测统一平差工作台', () => {
  ...
  test('十亿级高差、残差差异极小（显示同为 0.000）时只标红最大一条；相容时不标红', async ({ page }) => {
    ...
    const fillObs = async (row: number, f: string, e: string, dh: string, sigma: string) => {
      await page.getByLabel(`第 ${row} 行起点`).fill(f);
      await page.getByLabel(`第 ${row} 行终点`).fill(e);
      await page.getByLabel(`第 ${row} 行高差`).fill(dh);
      await page.getByLabel(`第 ${row} 行标准差`).fill(sigma);
    };
    await fillObs(1, 'A', 'B', '1000000000', '1');
    await fillObs(2, 'B', 'C', '1000000000', '2');
